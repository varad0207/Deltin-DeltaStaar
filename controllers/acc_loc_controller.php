<?php 
	...
	if (isset($_POST['submit'])) 
    {
        ...
        mysqli_query($conn,"INSERT INTO change_tracking_acc_locations(login,type,loc_id, location) values ('{$_SESSION['emp_id']}','Insert','$last_insert_id','$loc')");
    ...
    if (isset($_POST['update'])) 
    {
        ...
        echo $loc;
         ...
         if($AllowTrackingChanges){
            ...
            mysqli_query($conn,"insert into change_tracking_acc_locations (login,type,loc_id,location) 
                values ('{$_SESSION['emp_id']}','Update','{$row_affected['loc_id']}', '{$row_affected['location']}')");
         }
    ...
    if (isset($_GET['del'])) {
        ...
        if($AllowTrackingChanges){
            $row_affected=mysqli_fetch_array(mysqli_query($conn,"select * FROM acc_locations WHERE loc_id = $id"));
            mysqli_query($conn,"insert into change_tracking_acc_locations (login,type,loc_id,location) 
                values ('{$_SESSION['emp_id']}','Delete','{$row_affected['loc_id']}', '{$row_affected['location']}')");
        }
